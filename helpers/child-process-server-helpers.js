...
/**
 * Start the server in a child process. If a port number is not provided, an
 * available port is selected automatically.
 *
...
const start = (port) => {
  ...
  return getPort.then(port => new Promise((resolve, reject) => {
    const server = cproc.spawn('node', ['test-server.js', port, 'localhost']);
